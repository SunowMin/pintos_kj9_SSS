...
{
	char c = c_; // c를 int로 전달받는데, char로 형변환.

	ASSERT(string);

	for (;;)
		if (*string == c)
			return (char *)string;	// string 자체가 char 아닌가? 왜 char pointer를 적용하는 거지?
		else if (*string == '\0') // 문자열 끝까지 c를 찾지 못하면 NULL
			return NULL;
		else
			string++; // char인데 또 여기서는 ++를 하고 있네? 문자열 주소이기 때문에 인덱스를 높여준다.
}
