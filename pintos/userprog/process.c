...
tid_t process_create_initd(const char *file_name)
{
	...
	fn_copy = palloc_get_page(0); // 페이지의 시작 주소
	if (fn_copy == NULL)
		return TID_ERROR;
	strlcpy(fn_copy, file_name, PGSIZE);

	char buff[128];
	strlcpy(buff, file_name, 128);

	char *save_ptr, *file_token;

	file_token = strtok_r(buff, " ", &save_ptr);

	/* Create a new thread to execute FILE_NAME. */
	tid = thread_create(file_token, PRI_DEFAULT, initd, fn_copy); // fn_copy는 매개변수를 복사. initd로 그대로 전달한다.
	if (tid == TID_ERROR)
...
int process_exec(void *f_name) // f_name은 arguments. 파일명 - 인자1 - 인자2
{
	...
	// 새로운 프로그램을 실행할 때 intr_frame을 직접 만들어준다.
...
void argument_stack(char **arg_list, int idx, struct intr_frame *if_)
{
	...
	for (int i = idx - 1; i > -1; i--)
	{
		...
		{
			if_->rsp = if_->rsp - 1;												// 스택 주소 감소. 왜 1씩 감소하는 거지?
			memset(if_->rsp, arg_list[i][j], sizeof(char)); // 주소에 문자 저장. arg_list에 접근할 때 이중배열로 접근하는 이유, 한 문자(바이트)씩 스택에 저장하기 위해서
		}
		/* word-align */
		if (i == 0)
		{
