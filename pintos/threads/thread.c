...
thread_set_priority (int new_priority) {
	...
	// 우선순위를 갱신했으면 ready_list의 맨 앞 요소보다 우선순위가 낮아졌을 수도 있음
	struct thread *ready_list_front = list_entry(list_front(&ready_list), struct thread, elem);
	// ready_list의 맨 앞(가장 높은 priority) 스레드가 내 priority보다 높으면 
	if (ready_list_front->priority > thread_current ()->priority){
		// 또 양보해야 함
		thread_yield();
	}
}
...
// sleep_list에 삽입할 때 스레드가 일어날 시각(wakeup_tick) 기준으로 오름차순 정렬
...
// ready_list에 삽입할 때 priority 기준으로 내림차순 정렬
bool dsc_priority (const struct list_elem *x, const struct list_elem *y, const void *aux){
	struct thread *tx = list_entry(x, struct thread, elem);
	struct thread *ty = list_entry(y, struct thread, elem);
